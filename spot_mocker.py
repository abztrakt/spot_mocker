...
def scrub(item, item_type=""):
    ...
    item['extended_info'][
        's_description'] = description.gen_description(owner, item_type)
...
def create(item, item_type, items=[]):
    if item_type != "Tech":
        ...
        for i in ids:
            test = mock_path + "/api/v1/spot?item%3Aid=" + \
                str(i) + "&extended_info%3Aapp_type=tech"
            # print tech_item
...
def main():
    ...
    if ch == 1:
        item_type = raw_input(
            "First things first: What kind of spots are we viewing here (Food, Tech, Study)? ")
        ...
        for item_type, url in secrets.ITEMS.iteritems():
            ...
            indexes = random.sample(
                range(0, len(data) - 1), min(num, len(data) - 1))
            items = []
